-- Operational Task Management MVP: task tables
-- Dependencies: agent_runs table (created in 20260629151757)
...
-- MVP RLS decision:
-- RLS is intentionally not enabled for these task tables in the local/manual MVP
-- because full auth and role management are out of scope. The Vite app uses the
-- anon key for local review, so anon/authenticated need table privileges.
-- Do not deploy this policy posture to production before adding auth + RLS.
grant select, insert, update, delete on tasks to anon, authenticated;
grant select, insert, update, delete on task_targets to anon, authenticated;
grant select, insert, update, delete on task_steps to anon, authenticated;
grant select, insert, update, delete on task_comments to anon, authenticated;
grant select, insert, update, delete on task_logs to anon, authenticated;
