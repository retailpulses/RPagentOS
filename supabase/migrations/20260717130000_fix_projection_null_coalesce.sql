...
-- Rollback: restore prior view body without COALESCE changes

-- The product_commercials.presale_qty column is NULL for all rows in the
-- hosted database. The Worker API validates presale_qty as a required
-- non-negative integer and fails-closed on NULL. This migration adds
-- COALESCE guards for the two fields that may be NULL in production data.
