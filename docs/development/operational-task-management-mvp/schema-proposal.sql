-- Operational Task Management MVP schema proposal
-- Date: 2026-07-02
-- Status: proposal only; convert to a migration after review.

-- Locked MVP decisions:
-- - Use text columns with check constraints, not Postgres enum types.
-- - Use string-based owner fields; do not implement full auth.
-- - Use loose task target references; do not enforce product/listing/account FKs.
-- - Keep task_logs separate from agent_execution_logs.
...
create table if not exists tasks (
  ...
  constraint tasks_owner_key_check check (
    owner_key is null
    or owner_key in ('jim', 'agent_listing', 'agent_promotion', 'external_operator')
  ),
...
create index if not exists idx_tasks_platform_shop
  on tasks(platform, shop_code);

create index if not exists idx_tasks_owner
  on tasks(owner_type, owner_key);
...
create index if not exists idx_task_targets_task_id
  on task_targets(task_id);

create index if not exists idx_task_targets_target
  on task_targets(target_type, target_id);

create index if not exists idx_task_targets_ref_json
  on task_targets using gin(target_ref_json);
