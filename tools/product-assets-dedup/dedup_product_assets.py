...
def create_backup(supabase: Client):
    """Export product_assets to JSON backup file before mutation."""
    backup_file = Path(__file__).resolve().parent / f"{BACKUP_TABLE}.json"
    log(f"Creating backup: {backup_file}")

    if backup_file.exists():
        log(f"  Backup file already exists: {backup_file}")
        log(f"  Delete it first if you want a fresh backup.")
        return

    rows = fetch_all_assets(supabase)
    log(f"  Writing {len(rows)} rows to {backup_file}...")

    import json
    with open(backup_file, "w") as f:
        json.dump(rows, f, default=str)

    log(f"  Backup complete: {len(rows)} rows in {backup_file}")
    log(f"  Rollback: python3 -c 'from dedup_product_assets import restore_backup; restore_backup()'")
